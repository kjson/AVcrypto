...
# Set test variables
_log_name = 'hyper_elliptic_curve_arithmetic_test.log'
...
for prime in primes_less_than(_upper_bound_on_primes):
	...
	for i in xrange(0,num_curves):
		...
		try:
			...
			%s over F_%s. %s' % (H,prime,'EXCEPTION: ' + str(e)))
			fail_count+=1
			continue
		...
		try:
			...
			%s over F_%s. %s' % (P,Q,H,prime,'EXCEPTION: ' + str(e)))
			fail_count+=1
			continue
		...
		try:
			...
			%s over F_%s. %s' % (R,H,prime,'EXCEPTION: ' + str(e)))
			fail_count+=1
			continue
